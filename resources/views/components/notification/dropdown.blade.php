<div x-data="notificationDropdown()" class="relative">
    ...
    <button @click="toggleDropdown" class="p-1 rounded-full text-gray-500 hover:text-gray-700 focus:outline-none relative">
        ...
        <span x-show="unreadCount > 0" x-text="unreadCount > 9 ? '9+' : unreadCount" class="absolute -top-2 -right-2 flex items-center justify-center h-5 w-5 rounded-full bg-red-600 text-white text-xs font-bold shadow-sm"></span>
    </button>

    <!-- Notification Dropdown -->
    <div x-show="isOpen" @click.away="isOpen = false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute right-0 mt-2 w-80 bg-white rounded-md shadow-xl overflow-hidden z-50 border border-gray-200" style="display: none;">
        <div class="py-2">
            <div class="px-4 py-3 text-sm font-semibold text-gray-700 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                ...
                <span x-show="unreadCount > 0">
                    <button @click="markAllAsRead" class="text-green-600 hover:text-green-800 text-xs font-medium">
                        {{ __('Mark all as read') }}
                    </button>
                </span>
            </div>
                    ...
                    <div :class="{ 'bg-blue-50': !notification.is_read, 'hover:bg-gray-50': notification.is_read }" class="border-b border-gray-200 last:border-b-0 transition-colors duration-150">
                        <div class="px-4 py-3">
                            <div class="flex justify-between items-start">
                                <h4 class="text-sm font-medium" :class="!notification.is_read ? 'text-blue-800' : 'text-gray-800'" x-text="notification.title"></h4>
                                <span class="text-xs text-gray-500 ml-2 whitespace-nowrap" x-text="formatDate(notification.created_at)"></span>
                            </div>
                            <p class="mt-1 text-xs" :class="!notification.is_read ? 'text-blue-700' : 'text-gray-600'" x-text="notification.message"></p>
                            <div class="mt-2 flex justify-between items-center">
                                <template x-if="notification.link">
                                    <a :href="notification.link" class="text-xs font-medium text-green-600 hover:text-green-800">
                                        {{ __('View Details') }}
                                    </a>
                                </template>
                                <template x-if="!notification.link">
                                ...
                                <template x-if="!notification.is_read">
                                    <button @click="markAsRead(notification.id)" class="text-xs font-medium text-gray-600 hover:text-gray-900">
                                        {{ __('Mark as Read') }}
                                    </button>
                                </template>
                            </div>
            ...
            <div class="px-4 py-3 bg-gray-50 text-center border-t border-gray-200">
                <a href="{{ route('notifications.index') }}" class="inline-flex items-center text-sm text-green-600 hover:text-green-800 font-medium">
