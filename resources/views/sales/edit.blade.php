    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Edit Sale Information') }}: {{ $car->year }} {{ $car->make }} {{ $car->model }}
            </h2>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('sales.update', [$car, $sale]) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <x-text-input id="listing_date" class="block mt-1 w-full" type="date" name="listing_date" :value="old('listing_date', $sale->listing_date->format('Y-m-d'))" required />
                                    <x-input-error :messages="$errors->get('listing_date')" class="mt-2" />
                                </div>

                                <div>
                                    <x-input-label for="asking_price" :value="__('Asking Price (R)')" />
                                    <x-text-input id="asking_price" class="block mt-1 w-full" type="number" name="asking_price" :value="old('asking_price', $sale->asking_price)" min="0" step="0.01" required />
                                ...
                                <div>
                                    ...
                                    <select id="platform" name="platform" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring focus:ring-green-500 focus:ring-opacity-50" required>
                                        ...
                                        <option value="Dealership" {{ old('platform', $sale->platform) == 'Dealership' ? 'selected' : '' }}>{{ __('Dealership') }}</option>
                                        <option value="Online Marketplace" {{ old('platform', $sale->platform) == 'Online Marketplace' ? 'selected' : '' }}>{{ __('Online Marketplace') }}</option>
                                        <option value="Private Sale" {{ old('platform', $sale->platform) == 'Private Sale' ? 'selected' : '' }}>{{ __('Private Sale') }}</option>
                                        <option value="Auction" {{ old('platform', $sale->platform) == 'Auction' ? 'selected' : '' }}>{{ __('Auction') }}</option>
                                        <option value="Other" {{ old('platform', $sale->platform) == 'Other' ? 'selected' : '' }}>{{ __('Other') }}</option>
                                    </select>
                        ...
                        <div class="mb-6">
                                ...
                                <div class="flex">
                                    ...
                                    <div class="ml-3">
                                        ...
                                        <div class="mt-2 text-sm text-yellow-700">
                                            <p>{{ __('This car is currently marked as') }} <strong>{{ ucfirst($car->current_phase) }}</strong>. {{ __('If you are updating sale information because the car has been sold, please check the box below to update the car status.') }}</p>
                                        </div>
                                        <div class="mt-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <x-text-input id="selling_price" class="block mt-1 w-full" type="number" name="selling_price" :value="old('selling_price', $sale->selling_price)" min="0" step="0.01" />
                                    <x-input-error :messages="$errors->get('selling_price')" class="mt-2" />
                                </div>

                                <div>
                                    <x-input-label for="sale_date" :value="__('Sale Date')" />
                                    <x-text-input id="sale_date" class="block mt-1 w-full" type="date" name="sale_date" :value="old('sale_date', $sale->sale_date ? $sale->sale_date->format('Y-m-d') : now()->toDateString())" />
                                ...
                                <div>
                                    ...
                                    <x-text-input id="commission" class="block mt-1 w-full" type="number" name="commission" :value="old('commission', $sale->commission)" min="0" step="0.01" />
                                    <x-input-error :messages="$errors->get('commission')" class="mt-2" />
                                </div>

                                <div>
                                    <x-input-label for="fees" :value="__('Fees (R)')" />
                                    <x-text-input id="fees" class="block mt-1 w-full" type="number" name="fees" :value="old('fees', $sale->fees)" min="0" step="0.01" />
                                    <x-input-error :messages="$errors->get('fees')" class="mt-2" />
                                </div>

                                <div class="md:col-span-2">
                                    <x-input-label for="notes" :value="__('Notes')" />
                                    <textarea id="notes" name="notes" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring focus:ring-green-500 focus:ring-opacity-50">{{ old('notes', $sale->notes) }}</textarea>
                        ...
                        <div class="flex items-center justify-between mt-6">
                            <form action="{{ route('sales.destroy', [$car, $sale]) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this sale information?');">
                                @csrf
                                @method('DELETE')
